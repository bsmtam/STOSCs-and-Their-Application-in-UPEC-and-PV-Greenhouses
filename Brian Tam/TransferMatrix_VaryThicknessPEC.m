...
function TransferMatrix_VaryThicknessPEC
...
thicknesses = [1000   100  0 1000 1000 ];  % thickness of each corresponding layer in nm (thickness of the first layer, and layer which thickness is varied is irrelivant)
...
LayerVaried = 3; % index of material where thickness will be varied
...
fileName = append(string(layers(activeLayer)),'_no pt mesh');
saveFile = 1;
%
...
EmmottGVThickData = textscan(fopen('GvsThicknessEmmott.txt'), '%f %f', 'HeaderLines', 0); % for G calcs
emmottGVThick = [EmmottGVThickData{1}, EmmottGVThickData{2}];

EmmottJscThickData = textscan(fopen('JscVsThicknessEmmott.txt'), '%f %f', 'HeaderLines', 0); % for G calcs
...
%     fluxLambdaRange = emmottPhotonFlux(:,1);
    ...
    %------------Get Active Layer Thickness that is > 8.14 mA/cm2--------%
    
end

JscArray = [thicknessParameters.Jsc];
targetJsc = 8.1301;
[uniqueJsc, uniqueInd] = unique(JscArray);
minActiveLayerThickness = interp1(uniqueJsc, VaryThickness(uniqueInd),targetJsc, 'linear', 'extrap');


figure(1)
plot(VaryThickness, [thicknessParameters.Jsc], 'DisplayName', 'Modelled Max Jsc', 'LineWidth', 2)
if compareEmmott == true
    hold on
    plot(emmottJscThick(:,1),emmottJscThick(:,2), 'DisplayName', 'Emmott Model');
end
grid on
%legend
title('Max J_s_c obtained with 100% IQE')
xlabel('Active Layer Thickness (nm)')
ylabel('Current Density (mA/cm^2)')
ylin1 = yline(8.1301, '--k', {'8.1301 mA/cm2'},'LineWidth', 1.5);
ylin1.Annotation.LegendInformation.IconDisplayStyle = 'off';
ylin1.LabelHorizontalAlignment = 'left';
xlin1 = xline(minActiveLayerThickness, '--k', {'Min. Thickness =', append(string(minActiveLayerThickness), ' nm')},'LineWidth', 1.5);
...
figure(2)
plot(VaryThickness, [thicknessParameters.AVT].*100, 'DisplayName', 'AVT', 'LineWidth', 2);
% hold on 
% plot(VaryThickness, [thicknessParameters.HPT].*100, 'DisplayName', 'HPT');
% hold on
% plot(VaryThickness,[thicknessParameters.growthfactor].*100, 'DisplayName', 'Growth Factor');
ylabel('Average Transmission (%)')
title('Average Transmission (from 300-900nm) as a function of Hematite Thickness with Pt mesh')

if compareEmmott == true
    hold on
    plot(emmottGVThick(:,1), emmottGVThick(:,2), 'DisplayName', 'Emmott Growth Factor');
end
grid on
xlabel('Active Layer Thickness (nm)')
