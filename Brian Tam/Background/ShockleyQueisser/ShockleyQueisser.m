...
%% MAIN FUNCTION
function[data,Y] = ShockleyQueisser(EG,varargin)
...
maxError2 = 0;
    ...
    Jsc = conc * trapz(E, qy.*a.*photonFlux);
	...
	% J-V equation
    JV = @(x) ( Jsc - (Jemf+Jemr) * exp(x) - alpha*exp(beta*x) )
    
	% maximize the power P=J*V, set dP/dx = 0
	dPdx = @(x) ( Jsc - (1+x) * (Jemf+Jemr) * exp(x) - (1+beta*x) * alpha*exp(beta*x) );
    xop = fzero(dPdx, 42);
    if(beta==1 || alpha==0)
        xoc = log( Jsc /( Jemf + Jemr + alpha ) ); % simple formula
    else
        xoc = fzero(JV, 45);
    end
    Vop = kT * xop;
	...
	etaExt = Jemf * exp(xoc) /( Jemf*exp(xoc) + Jemr*exp(xoc) + alpha*exp(beta*xoc) );
    etaInt = intRad*exp(xoc) /( intRad*exp(xoc) + alpha*exp(beta*xoc) );
    
    % where is the peak of the re-emission spectrum?
	%	not correct for qy(E)~=1
    [~,ind] = max(alphaL.*b0);
    aint = 1 - a(ind) /( 4*nr^2*alphaL(ind) );
    avgBounce = 1/( 4*alphaL(ind) );
...
% 
 data = struct('Jsc',Jsc,'Voc',Voc,'Vop',Vop,'eff',eff,'FF',FF);%;,...
