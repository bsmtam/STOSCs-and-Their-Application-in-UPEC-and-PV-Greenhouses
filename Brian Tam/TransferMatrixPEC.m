...
% (Modeling photocurrent action spectra of photovoltaic devices based on
% organic thin films"
...
% Creates a matrix n with rows defined by no. of layers and columns defined 
% by the number of wavelength entries in lambda for complex refractive index of each layer
n = zeros(size(layers,2),size(lambda,2)); 
for index = 1:size(layers,2)
    n(index,:) = LoadRefrIndex(layers{index},lambda);%loads the indices for each layer from .txt files
end
t = thicknesses;
...
t(1)=0; %why is this set to zero? is this due to the first layer being glass/air?
t_cumsum=cumsum(t);
...
R=lambda*0;
E=zeros(length(x_pos),length(lambda));%creates a matrix for electric field for every x-position and for every wavelength
for l = 1:length(lambda)
    % Calculate transfer matrices for incoherent reflection and transmission at the first interface
    S=I_mat(n(1,l),n(2,l)); %calculates the interface matrix for 1st and 2nd layers
...
Transmission = Transmission.*ptMeshTransmission;
...
%     parasitic_abs_check = zeros(size(parasitic_abs));
    
    
    spectrumData = load('spectrum.mat');
    fluxLambda = spectrumData.fluxLambda; %flux lambda is in mA.cm-2.nm-1
    fluxLambdaRange = spectrumData.lambda;
    TransRange = 300:1:900;
    transInterpl = interp1(lambda, Transmission, TransRange, 'linear', 'extrap');
    transInterpl(transInterpl<0) = 0;
    transFlux = interp1(fluxLambdaRange.', fluxLambda.', TransRange);
    transNum = trapz(TransRange, transInterpl.*transFlux.*(1240./TransRange), 2);
    transDen = trapz(TransRange, transFlux.*(1240./TransRange), 2);
    avgTrans = transNum/transDen
    
%     RefRange = 900:1:2000;
%     refInterpl = interp1(lambda, Reflection, RefRange, 'linear','extrap');
%     refInterpl(refInterpl<0) = 0;
%     refFlux = interp1(fluxLambdaRange.', fluxLambda.', RefRange);
%     refNum = trapz(RefRange, refInterpl.*refFlux.*(1240./RefRange), 2);
%     refDen = trapz(RefRange, refFlux.*(1240./RefRange), 2);
%     avgRef = refNum/refDen
    

%     figure
%     plot(lambda, Transmission, 'DisplayName', 'Transmission');
%     hold on
%     plot(TransRange, transInterpl, 'DisplayName', 'Trans Interp');
%     xlabel('Wavelength (nm)')
%     
%     figure
%     plot(lambda, Reflection, 'DisplayName', 'Reflection');
%     hold on
%     plot(RefRange, refInterpl, 'DisplayName', 'Trans Interp');
%     xlabel('Wavelength (nm)')
%     
%     figure
%     for plotind = 1: length(layers)
%         
%         plot(lambda, real(n(plotind,:)), 'DisplayName', string(layers(plotind)));
%         
%         if plotind ~= length(layers)
%             hold on
%         end
%         
%     end
%     grid on
%     legend
%     title('n values for stack layers')
%     xlabel('Wavelength (nm)')
    
%     figure
%      for plotind = 1: length(layers)
%         if plotind == 2 || plotind == 3 || plotind == 5 || plotind == 8
%             plot(lambda, imag(n(plotind,:)), 'DisplayName', string(layers(plotind)));
%         end 
%         
%         if plotind ~= length(layers)
%             hold on
...
%     grid on
%     legend
%     title('k values for stack layers')
%     xlabel('Wavelength (nm)')
    

figure
plot(lambda, Transmission.*100, 'DisplayName', 'PEC Device Transmission', 'LineWidth', 2)
hold on
plot(lambda, Absorption(activeLayer, :).*100, 'DisplayName', 'PEC Active Layer Absorption', 'LineWidth', 2)
xlabel('Wavelength (nm)')
ylabel('(%)')
title('Absorption and Transmission profile of PEC')
legend
grid on
